...
static struct thread_info *io_threads;
...
static int file_backend_open(struct ubbd_backend *ubbd_b)
{
	...
	io_threads = calloc(ubbd_b->num_queues, sizeof(struct thread_info));
	if (!io_threads) {
		ret = -ENOMEM;
		goto close_fd;
	}
	...
	for (i = 0; i < ubbd_b->num_queues; i++) {
		struct thread_info *info = &io_threads[i];
...
static int file_backend_writev(struct ubbd_backend *ubbd_b, struct ubbd_backend_io *io)
{
	...
	if (io->sync) {
		...
		struct thread_info *info = &io_threads[io->queue_id % ubbd_b->num_queues];
...
static struct ubbd_backend_io *file_backend_create_backend_io(struct ubbd_backend *ubbd_b, uint32_t iov_cnt)
{
	struct file_backend_io *file_io;

	file_io = calloc(1, sizeof(struct file_backend_io) + sizeof(struct iovec) * iov_cnt);
	if (!file_io) {
		ubbd_err("failed to malloc file backend io\n");
		return NULL;
	}
